...
def product_page(product: dict[str, object], landing: dict[str, object],
    ...
    app_store_id = app.get("app_store_id") if app else None
    on_app_store = bool(app_store_id)
    if on_app_store:
        store_url = f"https://apps.apple.com/us/app/{esc(slug)}/id{esc(app_store_id)}"
        hero_note = (
            f"Available on the App Store for iOS. No verified public TestFlight invitation "
            f"link is published on this page. {esc(landing['privacy'])}"
        )
        actions = (
            f'<a class="product-button" href="{store_url}" rel="noopener">View on the App Store</a>'
            f'<a class="product-button product-button--secondary" href="#waitlist">Express interest</a>'
            f'<a href="#how-it-works">See how it works <span aria-hidden="true">↓</span></a>'
        )
        interest_subject = quote(f"{name} early access")
        waitlist = f"""<section id="waitlist" class="product-waitlist"><div><p>Early access</p><h2>Express interest in {esc(name)}.</h2><p>No account is created. Share your email if you would like details when early access opens.</p></div><form data-waitlist data-app-name="{esc(name)}" data-app-slug="{esc(slug)}" data-intake-endpoint="https://primandir.com/api/intake" action="mailto:support@priyanshchordia.com?subject={interest_subject}" method="post"><label>Email address <em>(required)</em><input type="email" name="email" autocomplete="email" placeholder="you@example.com" required></label><label>First name <em>(optional)</em><input type="text" name="first_name" autocomplete="given-name"></label><label>Device and iOS version, or what you want to try <em>(optional)</em><textarea name="device" aria-describedby="device-hint"></textarea><small id="device-hint">Helps prioritise invitations across devices. Leave blank if you would rather not say.</small></label><label class="product-consent"><input type="checkbox" name="consent" required> Email me about early access and occasional updates for {esc(name)}. I can unsubscribe at any time.</label><button class="product-button" type="submit">Express interest in {esc(name)}</button><p class="waitlist-status" role="status" aria-live="polite" tabindex="-1"></p><p class="waitlist-fallback">The signup service is not connected yet. Email <a href="mailto:support@priyanshchordia.com?subject={interest_subject}">support@priyanshchordia.com</a> with the subject “{esc(name)} early access” and you will be added by hand.</p></form></section>"""
        faq = f"""<section class="product-faq"><div class="product-section-heading"><p>Questions and limits</p><h2>{esc(faq_heading)}</h2></div><details><summary>Can I download {esc(name)} from the App Store?</summary><p>Yes. {esc(name)} is listed on the App Store for iOS. This page does not publish a separate TestFlight invitation link.</p></details><details><summary>What happens after I express interest?</summary><p>Your email is used only for follow-up about {esc(name)} feedback or early-access coordination when appropriate. There is no queue position or promised timeline.</p></details><details><summary>What do you do with my email address?</summary><p>It is used only to send early-access details and occasional updates. You can ask to be removed at any time.</p></details><div class="product-limits"><p>Honest limitations</p><strong>{esc(landing['limitation'])}</strong></div></section>"""
        footer_status = f"{esc(name)} is available on the App Store; no public TestFlight link is published here."
        nav_waitlist = "Express interest"
        fit_label = "Who it is for"
        features_heading = "What ships today."
    else:
        hero_note = f"In TestFlight beta — not on the App Store yet. {esc(landing['privacy'])}"
        actions = (
            f'<a class="product-button" href="#waitlist">Join the waitlist</a>'
            f'<a href="#how-it-works">See how it works <span aria-hidden="true">↓</span></a>'
        )
        waitlist = f"""<section id="waitlist" class="product-waitlist"><div><p>Waitlist</p><h2>Ask for a TestFlight invitation.</h2><p>No account is created. Your email is stored only to send {esc(name)} beta invitations.</p></div><form data-waitlist data-app-name="{esc(name)}" action="mailto:support@priyanshchordia.com?subject={subject}" method="post"><label>Email address <em>(required)</em><input type="email" name="email" autocomplete="email" placeholder="you@example.com" required></label><label>First name <em>(optional)</em><input type="text" name="first_name" autocomplete="given-name"></label><label>Device and iOS version, or what you want to test <em>(optional)</em><textarea name="device" aria-describedby="device-hint"></textarea><small id="device-hint">Helps prioritise invitations across devices. Leave blank if you would rather not say.</small></label><label class="product-consent"><input type="checkbox" name="consent" required> Email me about TestFlight access and occasional updates for {esc(name)}. I can unsubscribe at any time.</label><button class="product-button" type="submit">Join the {esc(name)} waitlist</button><p class="waitlist-status" role="status" aria-live="polite" tabindex="-1"></p><p class="waitlist-fallback">The signup service is not connected yet. Email <a href="mailto:support@priyanshchordia.com?subject={subject}">support@priyanshchordia.com</a> with the subject “{esc(name)} beta” and you will be added by hand.</p></form></section>"""
        faq = f"""<section class="product-faq"><div class="product-section-heading"><p>Questions and limits</p><h2>{esc(faq_heading)}</h2></div><details><summary>Can I download {esc(name)} from the App Store?</summary><p>No. {esc(name)} is in TestFlight beta and is not available on the App Store yet.</p></details><details><summary>How soon will I get an invitation?</summary><p>Invitations are sent in small groups as testing capacity opens. There is no queue position or promised timeline.</p></details><details><summary>What do you do with my email address?</summary><p>It is used only to send beta invitations and occasional updates. You can ask to be removed at any time.</p></details><div class="product-limits"><p>Honest limitations</p><strong>{esc(landing['limitation'])}</strong></div></section>"""
        footer_status = f"{esc(name)} is in TestFlight beta and is not available on the App Store."
        nav_waitlist = "Join waitlist"
        fit_label = "Who the beta is for"
        features_heading = "Everything in the beta build."
    body = f"""<article class="product-system product-system--{esc(slug)}" data-app="{esc(slug)}">
<header class="product-nav"><a class="product-wordmark" href="../../index.html">Priyansh Chordia <span>· iOS</span></a><nav aria-label="Product"><a href="../../index.html#work">Products</a><a href="#top">{esc(name)}</a><a href="#waitlist">{esc(nav_waitlist)}</a></nav></header>
<div id="top"><section class="product-hero"><div><p class="product-kicker">{esc(name)}</p><h1>{esc(positioning)}</h1><p class="product-hero-body">{esc(hero_body)}</p><p class="product-hero-note">{hero_note}</p><div class="product-actions">{actions}</div></div><div class="product-motif" aria-hidden="true"><span></span><i></i><b></b></div></section>
...
<section class="product-features"><div class="product-section-heading"><p>Feature highlights</p><h2>{esc(features_heading)}</h2></div><dl>{features}</dl></section>
<section class="product-fit"><div class="product-section-heading"><p>{esc(fit_label)}</p><h2>{esc(fit_heading)}</h2></div><div><article><h3>A good fit if you</h3><p>{esc(landing['audience'])}</p></article><article><h3>{esc(name)} is deliberately not</h3><p>{esc(landing['limitation'])}</p></article></div></section>
{waitlist}
{faq}
<footer class="product-footer"><p>More from this portfolio</p><h2>Independent iOS apps, built one at a time.</h2><div>{legal}<a href="../../index.html#work">All products</a><a href="mailto:support@priyanshchordia.com">support@priyanshchordia.com</a></div><small>© 2026 Priyansh Chordia. {footer_status}</small></footer></div></article>"""
    structured_data = {
        "@context": "https://schema.org",
